...
#[derivative(Debug)]
pub struct UintLessThanOrEquals {
...
#[typetag::serde]
impl Condition for UintLessThanOrEquals {
...
mod tests {
  ...
  fn test_is_met() {
    test_init();
    let condition = UintLessThanOrEquals {
      left: Box::new(1),
      right: Box::new(1),
    };
  ...
  fn test_serde() {
    ...
    assert_eq!(
      serialized,
      r#"{"type":"UintLessThanOrEquals","left":{"type":"Int","value":2},"right":{"type":"Int","value":1}}"#
    );
    let deserialized: UintLessThanOrEquals = serde_json::from_str(&serialized).unwrap();
